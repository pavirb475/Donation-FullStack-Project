...
const Admin = () => {
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-stone-100">
        <div className="text-center p-8 bg-white rounded-xl shadow-lg max-w-md w-full border-t-4 border-red-500">
          ...
          <button onClick={() => navigate('/login')} className="w-full bg-orange-600 text-white font-bold py-2 rounded">Go to Login</button>
        </div>
      </div>
    );
  }
  ...
  const SidebarItem = ({ icon: Icon, label, id }) => (
    <button 
      ...
      className={`w-full flex items-center px-6 py-4 text-left transition-colors ${activeTab === id ? 'bg-orange-700 border-l-4 border-white font-bold' : 'hover:bg-orange-800 border-l-4 border-transparent'}`}
    >
  ...
  return (
    <div className="flex h-screen bg-stone-100 overflow-hidden">
      ...
      {/* Sidebar */}
      <div className="w-64 bg-orange-900 text-white flex flex-col shadow-2xl flex-shrink-0">
        <div className="p-6 bg-orange-950 flex items-center justify-center">
          <span className="text-2xl font-extrabold tracking-widest text-orange-100">ADMIN<span className="text-orange-400">PANEL</span></span>
        </div>
        ...
        <main className="p-8">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-amber-500">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="p-3 bg-amber-100 text-amber-600 rounded-full"><FaUsers size={24} /></div>
                  </div>
                </div>
                <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-green-500">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="p-3 bg-emerald-100 text-emerald-600 rounded-full"><FaDonate size={24} /></div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 mt-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setActiveTab('campaigns')} className="p-4 border border-gray-200 rounded-lg hover:bg-stone-50 text-left transition">
                    <h4 className="font-bold text-orange-700">Quick Action: Manage Campaigns</h4>
                    <p className="text-sm text-gray-500 mt-1">Publish a new donation or volunteer drive.</p>
                  </button>
                  <button onClick={() => setActiveTab('donations')} className="p-4 border border-gray-200 rounded-lg hover:bg-stone-50 text-left transition">
                    <h4 className="font-bold text-emerald-700">Quick Action: View Finances</h4>
          ...
          {activeTab === 'campaigns' && (
            <div className="space-y-8">
              {/* Form (Create or Edit) */}
              <div className={`bg-white rounded-xl shadow-sm p-6 border-t-4 ${editingId ? 'border-yellow-500' : 'border-orange-500'}`}>
                ...
                <form onSubmit={handleSubmitCampaign} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Title</label><input type="text" required className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Category</label><input type="text" required className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Goal Amount (₹)</label><input type="number" required className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500" value={formData.goalAmount} onChange={e => setFormData({...formData, goalAmount: e.target.value})} /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Location</label><input type="text" className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} /></div>
                  
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Campaign Purpose</label>
                    <select 
                      className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500 bg-white"
                  ...
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Image URL</label><input type="url" placeholder="https://..." className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500" value={formData.image} onChange={e => setFormData({...formData, image: e.target.value})} /></div>

                  <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">Description</label><textarea required rows="3" className="w-full border-gray-300 rounded-md border p-2 focus:ring-orange-500 focus:border-orange-500" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} /></div>
                  <div className="md:col-span-2 flex justify-end">
                    <button type="submit" className={`${editingId ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-orange-600 hover:bg-orange-700'} text-white py-2 px-6 rounded-md font-bold shadow-md transition`}>
              ...
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-stone-50"><h2 className="text-xl font-bold text-gray-800">Active Campaigns</h2></div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {campaigns.map(c => (
                        <tr key={c._id} className="hover:bg-stone-50">
                          ...
                          <td className="px-6 py-4">
                            <div className="w-full bg-gray-200 rounded-full h-1.5 mb-1 max-w-[150px]"><div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${Math.min((c.raisedAmount / c.goalAmount) * 100, 100)}%` }}></div></div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap flex flex-col items-start gap-1">
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-orange-100 text-orange-800">{c.category}</span>
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-stone-100 text-gray-800 capitalize">{c.campaignType || 'combined'}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                            <button onClick={() => handleEditClick(c)} className="text-amber-600 hover:text-amber-900 bg-amber-50 p-2 rounded inline-flex items-center" title="Edit"><FaEdit /></button>
          ...
          {activeTab === 'users' && (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-stone-50"><h2 className="text-xl font-bold text-gray-800">Registered Users</h2></div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-stone-50">
                    <tr>
                    ...
                    {usersList.map(u => (
                      <tr key={u._id} className="hover:bg-stone-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${u.role === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-emerald-100 text-emerald-800'}`}>{u.role}</span>
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-500">
                          <div className="truncate max-w-[200px]">{u.skills?.join(', ') || 'None'}</div>
                          <div className="truncate max-w-[200px] text-xs text-orange-500">{u.interests?.join(', ') || ''}</div>
                        </td>
          ...
          {activeTab === 'donations' && (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-stone-50"><h2 className="text-xl font-bold text-gray-800">Donation Transactions</h2></div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-stone-50">
                    <tr>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {donations.map(d => (
                      <tr key={d._id} className="hover:bg-stone-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-emerald-600">₹{d.amount}</td>
                        <td className="px-6 py-4 whitespace-nowrap"><span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-emerald-100 text-emerald-800">{d.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {activeTab === 'volunteers' && (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-stone-50"><h2 className="text-xl font-bold text-gray-800">Volunteer Applications</h2></div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-stone-50">
                    <tr>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {volunteers.map(v => (
                      <tr key={v._id} className="hover:bg-stone-50">
